...
from your_package import (  # adjust import to your package name
...
def sample_nc(tmp_path):
    ...
    # second file with different values
...
def test_nc_reader_and_wind_speed(sample_nc):
    ...
    assert np.isclose(df_ws["wind_speed_10m [m/s]"].iloc[0], np.sqrt(13))

def test_nc_sorter_and_interpolation():
    # build synthetic df for 4 corners: lat ∈ [55.5,55.75], lon ∈ [7.75,8.0]
    ...
    # each corner has 2 rows
    assert len(c1)==2 and len(c4)==2
    # test interpolation at center lat=55.625 lon=7.875
    mid = interpolation(55.625,7.875,c1,c2,c3,c4)
    # should have same length as one corner
    assert len(mid)==2
    # wind_speed_10m should equal average of corners: mean(lat+lon)
...
def test_compute_power_law():
    ...
    assert f"wind_speed_50m" in out.columns
    # rough check: speeds scale between 10 and 100
    assert (out["wind_speed_50m"] > df["wind_speed_10m"]).all()

def test_fit_weibull_and_plot(tmp_path, capsys):
    # generate known Weibull data
    rng = np.random.default_rng(0)
    data = rng.weibull(2.0, size=1000)*8.0
